...
class db:
    def __init__(self):
        ...
        self.cc = pd.read_csv(r'C:\Users\korek\Desktop\X\Edukacja\Python\Module 15\Module 16.3\env\db\country_codes.csv',index_col=0)
        self.customers = pd.read_csv(r'C:\Users\korek\Desktop\X\Edukacja\Python\Module 15\Module 16.3\env\db\customers.csv',index_col=0)
        self.prod_info = pd.read_csv(r'C:\Users\korek\Desktop\X\Edukacja\Python\Module 15\Module 16.3\env\db\prod_cat_info.csv')

    @staticmethod
    def transation_init():
        transactions = pd.DataFrame()
        src = r'C:\Users\korek\Desktop\X\Edukacja\Python\Module 15\Module 16.3\env\db\transactions'
...
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[
                            ...
                            dcc.Tab(label='Rodzaj sklepów', value='tab-3')
                            ]),
...
def tab2_barh_prod_subcat(chosen_cat):
    ...
    ## tab3 callbacks
@app.callback(Output('barh-country','figure'),
            [Input('country_dropdown','value')])
def tab3_barh_store_type(chosen_cat):

    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['country']==chosen_cat)].pivot_table(index='country',columns='Store_type',values='total_amt',aggfunc='sum').assign(_sum=lambda x: x['Flagship store']+x['TeleShop']+x['e-Shop']+x['MBR']).sort_values(by='_sum').round(2)

    traces = []
    for col in ['Flagship store','TeleShop','e-Shop', 'MBR']:
        traces.append(go.Bar(x=grouped[col],y=grouped.index,orientation='h',name=col))
